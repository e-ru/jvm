...
public class CustomJwtAccessTokenConverter extends JwtAccessTokenConverter {
	...
	public OAuth2AccessToken enhance(OAuth2AccessToken accessToken, OAuth2Authentication authentication) {
		...
		result.setRefreshToken(customProperties.isReuseRefreshtoken()
				...
				: createRefreshToken(
						refreshTokenValiditySeconds(authentication.getOAuth2Request().getClientId())));
		return result;
	}

	int refreshTokenValiditySeconds(String clientId) {
		return jdbcClientDetailsService
				.loadClientByClientId(clientId)
				.getRefreshTokenValiditySeconds();
	}

	OAuth2RefreshToken createRefreshToken(int refreshTokenValiditySeconds) {
